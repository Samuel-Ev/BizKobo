# BizKobo — Every kobo counts.

An AI-powered bookkeeping, savings, and micro-credit platform — starting with NBU's
campus wallet as the pilot, built to extend to Nigerian market traders, artisans, and
informal SMEs more broadly.

Wallet, spending history, an explainable Trust Score, and Urgent 2K — an emergency
micro-loan feature underwritten by BizKobo's scoring and disbursed through a partner
bank (modeled here as **Zenith Bank**) — plus SME bookkeeping, Ajo/Esusu group savings,
and parents' financial monitoring.

Built multi-tenant from day one: every user, transaction, and loan is scoped
to a `university_id`. Adding a second school — or eventually a non-campus segment
like a market or trade union — means adding one tenant row and a brand color/logo,
not a rebuild.

**Branding:** logo and color palette (deep forest green `#214E35`, gold `#EFB63F`,
olive `#96B13D`) pulled directly from the BizKobo brand assets — see
`frontend/public/logo-mark.png` and `frontend/public/logo-full.png`.
...
## Deploying it so anyone with a link can use it

This is the setup to make it accessible from any phone/laptop, not just your machine —
both hosts below have free tiers.

### 1. Backend → Render

1. Push the `backend` folder to a GitHub repo (or the whole `bizkobo` folder — Render lets
   you point at a subdirectory).
2. On [render.com](https://render.com): New → Web Service → connect your repo.
3. Settings:
   - **Root directory**: `backend`
   - **Build command**: `pip install -r requirements.txt`
   - **Start command**: `bash start.sh`
   - **Environment variable**: `JWT_SECRET` — set this to a long random string
     (don't leave the dev default from `.env.example`)
4. Deploy. You'll get a URL like `https://bizkobo-api.onrender.com`.

Note: `start.sh` reseeds the demo account on every deploy, since Render's free tier
disk is wiped on redeploy — expected for a demo, not something you'd want once real
users have real data (switch to Postgres + a persistent disk before that point).

### 2. Frontend → Vercel

1. Push the `frontend` folder to a repo (or same repo, Vercel lets you set a root
   directory too).
2. On [vercel.com](https://vercel.com): New Project → import the repo.
3. **Root directory**: `frontend`
4. **Environment variable**: `NEXT_PUBLIC_API_URL` = your Render backend URL from step 1
   (e.g. `https://bizkobo-api.onrender.com`)
5. Deploy. You'll get a URL like `https://bizkobo.vercel.app` — that's the link you can
   send to anyone, on any device.

The backend's CORS is already configured to accept any `*.vercel.app` origin automatically
(see `backend/app/main.py`) — deliberately permissive for this demo phase, worth tightening
to your exact domain before this ever touches real money.
