    ...
    <div>
      ...
      {showCreate && (
        <motion.form
          ...
          className="rounded-xl2 border border-black/[0.07] bg-ink-2 p-4 mb-5 space-y-3 overflow-hidden"
        >
          <input
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder="Group name — e.g. Roommates Ajo"
            className="w-full bg-ink-3 border border-black/[0.09] rounded-xl px-3.5 py-2.5 text-[13.5px] text-paper placeholder-slate focus:outline-none focus:border-copper-light/60"
          />
          <div className="flex gap-3">
            <div className="flex-1">
              ...
              <input
                ...
                className="w-full bg-ink-3 border border-black/[0.09] rounded-xl px-3.5 py-2.5 text-[13.5px] text-paper focus:outline-none focus:border-copper-light/60"
              />
            </div>
            <div className="flex-1">
      ...
      {error && (
        <div className="text-[12.5px] text-red-700 bg-red-50 border border-red-200 rounded-xl px-3.5 py-2.5 mb-4">
          {error}
        </div>
      )}

      {myGroups.length === 0 && !showCreate ? (
        <div className="flex flex-col items-center text-center py-10 px-6 rounded-xl2 border border-dashed border-black/[0.09] mb-6">
        ...
        <div className="space-y-3 mb-7">
          {myGroups.map((g) => (
            <div key={g.id} className="rounded-xl2 border border-black/[0.07] bg-ink-2 p-4">
        ...
        <>
          ...
          <div className="space-y-3">
            {discoverGroups.map((g) => (
              <div key={g.id} className="rounded-xl2 border border-black/[0.07] bg-ink-2 p-4 flex items-center justify-between">
                <div>
                  <div className="font-medium text-paper text-[13.5px]">{g.name}</div>
                ...
                <button
                  ...
                  disabled={busyGroupId === g.id}
                  className="bg-black/[0.045] hover:bg-black/[0.05] text-paper text-[12.5px] font-semibold rounded-lg px-3.5 py-2 disabled:opacity-50"
                >
                  Join
                </button>
              </div>
